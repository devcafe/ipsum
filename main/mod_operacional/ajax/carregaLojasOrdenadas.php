<?php
	...
	if($op == ''){
		...
		if($ordemLojas == 1) {
			...
			$limite = $pdo->prepare("Select * From ipsum_operacionallojas a Inner Join ipsum_operacionalbandeiras b On (a.idEstabBandeira = b.idBandeira) ORDER BY idLoja DESC LIMIT $inicio,$total_reg");
	...
	if($op != "withFields" && $op != 'withFieldsFiltro'){
		...
		$lista .= '<table id = "lojasTable">';
			$lista .= '<tr>';
				if($ordemLojas == 1){
					$lista .= '<td id = "idLojaOrd"> Cod. Loja <img src="resources/images/Operacional/sortDown.png"></td>';
				}else {
					$lista .= '<td id = "idLojaOrd"> Cod. Loja <img src="resources/images/Operacional/sortup.png"></td>';
				}
				$lista .= '<td> CNPJ </td>';
